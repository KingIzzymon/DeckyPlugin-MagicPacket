#!/bin/bash
# Source: https://github.com/scawp/Steam-Deck.Wake-Up-Rig/blob/main/zWakeUpRig.sh
...
if [ ! -f "$config" ] || [ "$1" = "config" ]; then
  while read -r line; do
    ...
    echo "${column[0]}"$'\t'"${column[4]}" >> "$temp_arp" #Add hostnames #wip
  done <<< "$(ip n)"
  ...
  echo "$mac_address" > "$config" #dev
  mac_address_fwd="$mac_address"
  #call config file editor function #wip
else
  if [ ! -z "$1" ] ; then
    mac_address="$1"
  else
  ...
  # Source: https://stackoverflow.com/questions/31588035/bash-one-line-command-to-send-wake-on-lan-magic-packet-without-specific-tool
